...
impl Game {
    ...
    fn min_bag(&self) -> Choice {
        let mut bag = Choice{ red: 0, green: 0, blue: 0};
        for c in self.rounds.iter() {
            if c.red > bag.red {
                bag.red = c.red;
            }
            if c.green > bag.green {
                bag.green = c.green;
            }
            if c.blue > bag.blue {
                bag.blue = c.blue;
...
fn main() {
    ...
    let measured_power = ret.games.iter().map(|x| x.min_bag().power()).reduce(|a, b| a+b).unwrap();
    println!("measured power: {}", measured_power);
}
...
".lines().map(String::from).collect();
        ...
        let measured_power: Vec<_> = ret.games.iter().map(|x| x.min_bag().power()).collect();
        println!("measured power: {:?}", measured_power);
