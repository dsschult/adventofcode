...
impl Party {
    ...
    fn top(&self, n: usize) -> Vec<Elf> {
        let mut sorted = self.members.to_vec();
        sorted.sort_by_key(|x| x.sum());
        let mut ret = sorted.split_off(sorted.len()-n);
        ret.reverse();
        ret
    }
}
...
fn main() {
    ...
    let top = ret.top(3);
    println!("Top: {:?}", top);
    println!("Top 3 calories sum: {}", top.iter().map(|x| x.sum()).reduce(|a,b| a+b).unwrap());
}
...
".lines().map(String::from).collect();
        ...
        let top = elves.top(3);
        println!("Top: {:?}", top);
        assert!(top[0].index == 4);
        assert!(top[1].index == 3);
        assert!(top[2].index == 5);
    }
}