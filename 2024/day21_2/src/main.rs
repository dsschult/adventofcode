...
impl Codes {
    ...
    fn new25(lines: &Vec<String>) -> Codes {
        Codes{
            data: lines.iter().map(|x| x.trim().to_string()).filter(|x| x.len() > 0).collect::<Vec<_>>(),
            keypads: vec![
                Keypad::get_num_keypad(),
                ...
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(), // 5
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(), // 10
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(), // 15
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(), // 20
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(),
                Keypad::get_dir_keypad(), // 25
            ],
        }
    }
    ...
    fn get_keys_for_code(&self, code: &String) -> String {
        ...
        let mut find_moves_memoized = |keypad_index: usize, start: char, end: char| -> Vec<String> {
            let keypad_type = match self.keypads[keypad_index] {
                Keypad::DirKeypad { keys: _ } => 1,
                Keypad::NumKeypad { keys: _ } => 2,
            };
            match memoize_keypads.get(&(keypad_type, start, end)) {
                None => {
                    let ret = self.keypads[keypad_index].find_moves(start, end);
                    memoize_keypads.insert((keypad_type, start, end), ret.clone());
                    ret
                },
                Some(ret) => ret.to_vec(),
            }
                ...
                true => {
                    match keypad_index == max_keypad {
                        ...
                        false => {
                            ...
                            println!("+keypad {}, len {}, queue len {}", new_keypad_index, cur_str.len(), queue.len());
                            //println!("  str: {}", cur_str);
